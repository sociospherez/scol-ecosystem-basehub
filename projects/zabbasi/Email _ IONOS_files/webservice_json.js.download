...
pfx.ws.json.BaseStub.prototype.getURI=function() {
   if(this._uri==null) {
      ...
      this._uri=window.location.protocol+"//"+window.location.host+reqpath;
   }
   return this._uri;
};
...
pfx.ws.json.Call.prototype.invoke=function(method,args,expLen) {
   ...
   for(var i=0;i<argLen;i++) {
     classString = Object.prototype.toString.call(args[i]);

     if (classString == "[object Array]") {
      jsonParams[i] = Array.prototype.slice.call(args[i], 0);
     } else {
      jsonParams[i]=args[i];
     }
   }
